...
export default function AudioMixing({ project }) {
  ...
  const [solo, setSolo] = useState(null); // key or null
  ...
  const playAll = () => {
    STEMS.forEach(s => audioRefs.current[s.key]?.play().catch(() => {}));
    setPlaying(true);
  };
  const pauseAll = () => {
    STEMS.forEach(s => audioRefs.current[s.key]?.pause());
    setPlaying(false);
  };
      ...
      <div className="glass rounded-xl p-5 space-y-4">
        ...
        <div className="flex items-center gap-3 pt-2 border-t border-border-subtle">
          ...
          <input type="range" min="0" max="100" value={masterVol}
            ...
            className="flex-1 h-2 bg-surface-sunken rounded-full appearance-none cursor-pointer
            ...
            <motion.div key={s.key} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.06 }}
              ...
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  <button onClick={() => toggleMute(s.key)}
                    className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all ${isMuted && !solo ? "bg-red-500/20 text-red-400" : "bg-surface-sunken text-content-muted hover:text-content-primary"}`}>
                    {muted[s.key] ? "M" : "M"}
                  </button>
                  <button onClick={() => toggleSolo(s.key)}
                    className={`px-2.5 py-1 rounded-lg text-xs font-medium transition-all ${isSoloed ? "text-content-inverse" : "bg-surface-sunken text-content-muted hover:text-content-primary"}`}
              ...
              <div className="flex items-center gap-3">
                ...
                <input type="range" min="0" max="100" value={isMuted ? 0 : volumes[s.key]}
                  ...
                  style={{ "--tw-slider-color": `var(${s.colorVar})` }} />
